jQuery(document).ready(function($){
  ...
  var $tabTitle = $(".members-which-tab");
  var $tabInput = $("input#perm_panel");
  ...
  function hasIndeterminateState( element ) {
    var $el = $( element );

    if ( !$el.is( "[role=treeitem]" ) ) {
      $el = $el.parents( "[role=treeitem]" ).first();
    }

    var $checkboxes = $el.find( "input[type=checkbox]" );
    var $checked = $checkboxes.filter( ":checked" );

    // keep in mind that the parent checkbox is also count
    return (
      ( $checkboxes.length - 1 ) <=  $checked.length
        && $checked.length !== 0
    );
  }

  function getRelatedCheckbox( element ) {
    var $el = $( element );
    ...
    return checkboxes.find(function ( el ) {
      var $checkbox = $( el );

      // keep the one whos closest parent
      // is $parent
      return (
        $checkbox
  ...
  $("body").on("click", "[role=tree] input[type=checkbox]", function ( evt ) {
      var $el = $( evt.target );
      var $parent = $el.parents( "[role=treeitem], [role=group]" ).first();
      var checked = $el.prop( "checked" );

      // item is a leaf
      if ( !$parent.is( "[role=treeitem]" ) ) {
        var $parents = $el.parents( "[role=treeitem]" );
        var $checkboxes = (
          $parents
            .map(function () {
              return $( getRelatedCheckbox( this ) );
            })
        );
      ...
      $parent
        .find( "input[type=checkbox]" )
        .prop( "checked", checked )
        // remove possible indeterminate state
        .removeAttr( "aria-checked" );
  });

  $("body").on("focus", "[role=tab]", function (e) {
    ...
    if ($panel) {
      ...
      $tabTitle.text(label);
      $tabInput.val($panel.attr("data-post-type"));
    }
  });
